...
import {
  ...
  FAQ_CTA_URL,
  faqPlainText,
} from "../src/data/faqPresaleAgent.js";
...
const REDIRECT_EXACT: Record<string, string> = {
  ...
  // Legacy booking short-link — now handled by the on-site booking form.
  "/meetuzair/quick-call": "/call",
  "/agents": "/",
  "/book": "/call",
  "/en": "/",
...
export const STATIC_META: Record<string, Meta> = {
  ...
  "/faq-presale-agent": { title: FAQ_PRESALE_AGENT_TITLE, description: FAQ_PRESALE_AGENT_DESCRIPTION, image: DEFAULT_IMAGE },
...
/** Escape text for the crawler HTML body. */
function faqEsc(t: string): string {
  return t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

/** Inline [label](/path) -> <a href="/path">label</a> for the crawler body. */
function faqAnswerHtml(answer: string): string {
  return faqEsc(answer).replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    (_m, label, href) => `<a href="${href}">${label}</a>`,
  );
}

const FAQ_AGENT_BODY =
  `<h1>${faqEsc(FAQ_PRESALE_AGENT_H1)}</h1><p>${faqEsc(FAQ_PRESALE_AGENT_INTRO)}</p>` +
  FAQ_PRESALE_AGENT.map(
    (f) =>
      `<section><h2>${faqEsc(f.question)}</h2><p>${faqAnswerHtml(f.answer)}</p>` +
      `<p><a href="${FAQ_CTA_URL}">${faqEsc(f.cta)}</a></p></section>`,
  ).join("");

const STATIC_BODY: Record<string, string> = {
  "/faq-presale-agent": FAQ_AGENT_BODY,
...
const TRACK_RECORD_LINE =
  "450+ families helped · $250M+ in new homes · 4.9 stars on Google.";
...
export const FUNNEL: Record<string, FunnelPage> = {
  "/new-to-presale-start-here": {
    ...
    faqs: [
      { q: "What is a presale condo in BC?", a: "A presale (new construction) condo is a new home you buy directly from the developer before it's built — often 12 to 36 months before completion. Staged deposits, close at completion. Protected in BC by a Disclosure Statement and a 7-day rescission window." },
  ...
  "/best-presale-realtor-fraser-valley": {
    ...
    intro: "I'm Uzair Muhammad — Fraser Valley's presale specialist who represents buyers, not developers. 450+ families helped, $250M+ in sales volume, 5 years focused on presale, 4.9 stars on Google. I don't represent developers, ever — only buyers. If you're thinking about a presale condo or townhome anywhere in the Fraser Valley, talk to me before you set foot in a developer's sales centre.",
    sections: [
  ...
  "/how-i-help": {
    ...
    sections: [
      ...
      { h: "Languages and track record", p: "English, Hindi, Urdu and Punjabi. 450+ families helped, $250M+ in sales volume, 5 years focused on presale, 4.9 stars on Google. Buyers, not developers — always." },
    ],
    faqs: [
...
export async function resolve(pathname: string, env: Record<string, string | undefined>): Promise<Resolved> {
  ...
  if (STATIC_META[path]) {
    ...
    const schema =
      path === "/"
        ? jsonLd(faqPage(HOME_FAQS))
        : path === "/faq-presale-agent"
          ? jsonLd(faqPage(FAQ_PRESALE_AGENT.map((f) => ({ q: f.question, a: faqPlainText(f.answer) })))) +
            jsonLd(breadcrumb("Presale Agent FAQ", "/faq-presale-agent/"))
          : "";
