      ...
      <div className="container-xl px-4 sm:px-6">
        ...
        <div className="text-center mb-10 sm:mb-14">
          <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-black text-foreground uppercase tracking-tight mb-3">
            400+ Units Sold to Investors & First-Time Buyers
          </h2>
        ...
        <div className="mb-10 sm:mb-14">
          ...
          <div className="sm:hidden">
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory px-4 -mx-4">
              {testimonials.map((testimonial, index) => <a key={index} href={GOOGLE_BUSINESS_URL} target="_blank" rel="noopener noreferrer" className={`flex-shrink-0 w-[300px] rounded-xl p-5 snap-center cursor-pointer transition-all hover:scale-[1.02] hover:shadow-lg ${testimonial.highlight ? 'bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20' : 'bg-card border border-border'}`}>
                  {/* Header: Photo + Name + Badge */}
                  <div className="flex items-center gap-3 mb-4">
                    <img src={testimonial.photo} alt={testimonial.name} className="w-12 h-12 rounded-full object-cover border-2 border-primary/30" />
          ...
          <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
            {testimonials.map((testimonial, index) => <a key={index} href={GOOGLE_BUSINESS_URL} target="_blank" rel="noopener noreferrer" className={`rounded-xl p-6 transition-all hover:scale-[1.02] hover:shadow-lg cursor-pointer block ${testimonial.highlight ? 'bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20' : 'bg-card border border-border'}`}>
                {/* Header: Photo + Name + Badge */}
                <div className="flex items-center gap-3 mb-4">
                  <img src={testimonial.photo} alt={testimonial.name} className="w-12 h-12 rounded-full object-cover border-2 border-primary/30" />
                  <div className="flex-1 min-w-0">
