...
function renderMarkdown(content: string): string {
  ...
  lines.forEach((line) => {
    ...
    if (line.startsWith("### ")) { closeList(); html.push(`<h3 class="blog-h3">${inline(line.slice(4))}</h3>`); return; }
    if (line.startsWith("#### ")) { closeList(); html.push(`<h3 class="blog-h3">${inline(line.slice(5))}</h3>`); return; }
    if (line.startsWith("- ") || line.startsWith("* ")) {
      if (!inList || listType !== "ul") { closeList(); html.push('<ul class="blog-ul">'); inList = true; listType = "ul"; }
      html.push(`<li>${inline(line.slice(2))}</li>`); return;
    }
    if (line.match(/^\d+\.\s/)) {
      if (!inList || listType !== "ol") { closeList(); html.push('<ol class="blog-ol">'); inList = true; listType = "ol"; }
      html.push(`<li>${inline(line.slice(line.indexOf(" ") + 1))}</li>`); return;
    }
    if (line.trim() === "") { closeList(); return; }
    closeList();
    if (!firstParagraphDone) { firstParagraphDone = true; html.push(`<p class="blog-p blog-dropcap">${inline(line)}</p>`); }
      ...
      <main style={{ background: "hsl(40 20% 97%)" }}>
        ...
        <section className="w-full pb-0" style={{ background: "#fff" }}>
          ...
          <div className="container-xl max-w-4xl pt-28 pb-0">
            ...
            <Link
              to="/blog"
              className="inline-flex items-center gap-2 mb-8 text-xs font-bold tracking-[0.15em] uppercase transition-colors w-fit hover:opacity-70"
